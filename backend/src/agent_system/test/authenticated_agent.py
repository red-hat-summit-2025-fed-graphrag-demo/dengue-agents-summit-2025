"""
Authenticated Test Agent

A simple agent that uses a tool that requires authentication.
"""
import logging
...
class AuthenticatedAgent(BaseAgent):
    """
    A simple agent that demonstrates using a tool that requires authentication.
    """
    
    def __init__(self, agent_id: str, config: Dict[str, Any], **kwargs):
        """
        Initialize the AuthenticatedAgent.
        ...
        self.auth_tool_id = "auth_hello_tool"
    ...
    async def _execute_processing(
        ...
        """
        Core processing logic for the AuthenticatedAgent.
        ...
        username = message.metadata.get("username", "Authenticated User")
        
        # Use the authenticated hello tool (requires auth)
        try:
            logger.info(f"Using authenticated hello tool for user: {username}")
            hello_result = await self.use_tool(
                tool_id=self.auth_tool_id,
                method_name="execute",
                name=username
            )
            
            # Log the result
            logger.info(f"Received greeting from authenticated tool: {hello_result}")
            
            # Prepare context for LLM
            context = {
                "greeting": hello_result.get("greeting", "Hello!"),
                "message": hello_result.get("message", "No additional message"),
                "authenticated": hello_result.get("authenticated", True),
            }
            
            # Prepare messages for the LLM
            messages = [
                Message(role=MessageRole.SYSTEM, content=system_prompt),
                Message(role=MessageRole.USER, content=f"Process this authenticated greeting: {message.content}"),
            ...
            # Set the next agent ID (none, as this is the end of our workflow)
            next_agent_id = None
            
            return response, next_agent_id
            
        except Exception as e:
            logger.error(f"Error in AuthenticatedAgent: {str(e)}", exc_info=True)
            error_message = Message(
                role=MessageRole.ASSISTANT,
                content=f"I encountered an authentication error: {str(e)}",
                metadata=message.metadata.copy()
            )
