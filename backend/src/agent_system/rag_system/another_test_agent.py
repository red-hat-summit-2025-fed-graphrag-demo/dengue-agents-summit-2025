"""
Another Test RAG Agent

A specialized agent for another test functionality
...
class AnotherTestAgent(BaseAgent):
    ...
    This class implements the Another Test RAG Agent functionality by:
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], registry_factory=None, **kwargs):
        """
        Initialize the AnotherTestAgent.
        ...
        self.prompt_id = config.get("prompt_id", "rag.another_test")
        
        logger.info(f"Initialized AnotherTestAgent with prompt_id: {self.prompt_id}")
    ...
    async def _execute_processing(
        ...
        """
        Core processing logic for the AnotherTestAgent.
        ...
        prompt_id = self.config.get("prompts", {}).get("system", {}).get("id", "rag.another_test")
