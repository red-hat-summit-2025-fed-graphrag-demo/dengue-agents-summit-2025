...
from src.agent_system.core.base_agent import BaseAgent  # noqa
from src.agent_system.core.message import Message, MessageRole  # noqa
from src.registries.base_registry import BaseRegistry  # noqa
from src.registries.prompt_registry import PromptRegistry  # noqa
...
# mypy: ignore-errors
# fmt: off
class {{ class_name }}(BaseAgent):  # noqa: E999, F821
    """
    {{ agent_description }}
    
    This class implements the {{ agent_name }} functionality by:
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], registry_factory=None, **kwargs):
        ...
        if "model_config" not in config:
            config["model_config"] = {
                "model_type": "{{ model_type }}",  # Use appropriate model type
        ...
        self.prompt_id = config.get("prompt_id", "{{ prompt_id }}")
        
        logger.info(f"Initialized {{ class_name }} with prompt_id: {self.prompt_id}")
    ...
    async def _execute_processing(
        ...
        """
        Core processing logic for the {{ class_name }}.
        ...
        prompt_id = self.config.get("prompts", {}).get("system", {}).get("id", "{{ prompt_id }}")
